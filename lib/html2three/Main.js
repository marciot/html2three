...
function tryReferringVR() {

    if(navigator.activeVRDisplays) {
            console.log("Found active VR display: ", activeVRDisplays.length);
    }

    /* EXPERIMENTAL: Support hyperlinking according to this article:
    ...
    try {
        navigator.vr.getReferringDisplays().then(function (displays) {
            if(!displays.length) {
                console.log("ReferringDisplays API exists, but no referral");
                tryNewVR();
                return;
            }
            vrDisplay = displays[0];
            console.log("ReferringDisplays exists. Starting VR presentation.");
            HTML2VR.startVR();
        });
        return true;
    } catch(e) {
        console.log("ReferringDisplays API seems non-existant, ignoring");
    }
}
    ...
    try {
        ...
        navigator.getVRDisplays().then(
            function(displays) {
                ...
                console.log("Is Presenting?", vrDisplay.isPresenting);
