...
var vrDisplay, vrElements, container, vrMode, leapController, scene;
...
function updatePose() {
    ...
    displacement.copy(headsetPose);
    displacement.sub(origin);
    
    const maxDistanceFromOrigin = 0.3; 
    
    if(displacement.length() > maxDistanceFromOrigin) {
        origin.copy(headsetPose);
        displacement.set(0,0,0);
    }
    
    // Update camera location
    cameraParent.rotation.setFromQuaternion(headsetOrientation);
    cameraParent.position.copy(displacement);
}
